...
def split_paragraphs_at_br(doc):
    """Tách triệt để một đoạn văn thành nhiều đoạn văn nếu nó chứa ngắt dòng mềm (Soft Return / w:br) hoặc ký tự xuống dòng \\n."""
    import copy
    import re
    from docx.oxml import OxmlElement
    ...
    for p in paragraphs:
        ...
        for child in list(p_elem):
            if child.tag == qn('w:pPr'):
                ...
                has_newline_in_text = (t_el is not None and t_el.text and ('\n' in t_el.text or '\r' in t_el.text))
                
                if not brs and not has_newline_in_text:
                    ...
                    for r_child in list(child):
                        if r_child.tag == qn('w:rPr'):
                        ...
                        elif r_child.tag == qn('w:t') and r_child.text and ('\n' in r_child.text or '\r' in r_child.text):
                            ...
                            for line_idx, line in enumerate(text_lines):
                                ...
                                new_r.append(new_t)
                                current_p.append(new_r)
                        else:
                            new_r = OxmlElement('w:r')
                            if rPr is not None:
                                new_r.append(copy.deepcopy(rPr))
                            new_r.append(copy.deepcopy(r_child))
                            current_p.append(new_r)
            else:
                current_p.append(copy.deepcopy(child))
...
def convert_all_question_tables_to_paragraphs(doc):
    """Chuyển đổi tất cả các bảng chứa câu hỏi (nhưng không phải bảng True/False) thành các đoạn văn bình thường."""
    import re
    tables_to_process = []
    ...
    for table in doc.tables:
        ...
        for row in table.rows:
            for cell in row.cells:
                text = cell.text.strip()
                if re.match(r'^[\s.:\-*]*(Câu|Questions?|Bài|câu|bài)\s*(hỏi\s+)?(\d+)', text, re.IGNORECASE):
                    has_question = True
                if INLINE_MC_RE.search(text):
                    has_choices = True
                    
        if has_question:
            # Nếu bảng có >= 3 cột và không chứa các lựa chọn trắc nghiệm inline (A. B. C. D.)
            # Thì nó là bảng True/False, sẽ được xử lý riêng bởi break_tf_tables_to_paragraphs!
            is_tf = (len(table.columns) >= 3 and not has_choices)
            if not is_tf:
                tables_to_process.append(table)
...
QUESTION_RE = re.compile(r'^[\s.:\-*]*(Câu|Questions?|Bài|câu|bài)\s*(hỏi\s+)?(\d+)\s*(?:\([^)]*\))?\s*[:.\-)?\s]?', re.IGNORECASE)
PLAIN_NUMBER_RE = re.compile(r'^[\s.:\-*]*(\d+)\s*(?:\([^)]*\))?\s*[.:)]\s*(?!\d)')
SECTION_RE = re.compile(
    r'^\s*(?:'
    r'(phần|part)\s*([IVXLCDM]+|\d+|[A-D])'
    r'|'
    r'([IVXLCDM]+|\d+|[A-D])\s*[.:]\s*(?=.*?(?:trắc nghiệm|tự luận|đúng[\s/]*sai|trả lời ngắn|chọn đ\s*\())'
    r')', re.IGNORECASE
)

INLINE_MC_RE = re.compile(r'(?<![A-Z])(?:\()?(\*?\s*[A-H]\s*\*?)\s*[.:)/\-]\s*\*?')
INLINE_TF_RE = re.compile(r'(?<![a-zA-Z])(?:\()?(\*?\s*[a-h]\s*\*?)\s*[.:)/\-]\s*\*?')
...
def get_answer_for_question(answer_keys: Dict[int, Dict[int, str]], section_idx: int, q_num: int, offsets: Optional[Dict[int, int]] = None) -> Optional[str]:
    ...
    if section_idx in answer_keys and q_num in answer_keys[section_idx]:
        ans = answer_keys[section_idx][q_num]
    else:
        ...
        for sec_idx in [section_idx, 1, 2, 3]:
            if sec_idx in answer_keys and abs_q_num in answer_keys[sec_idx]:
                ans = answer_keys[sec_idx][abs_q_num]
                break
                
    if ans:
        ans = ans.strip()
        m = re.match(r'^([A-H])\s*\(.*?\)$', ans, re.IGNORECASE)
        if m:
            ans = m.group(1).upper()
...
def _preprocess_split_runs(p, split_indices):
    """Tiền xử lý: Tách một run thành các run nhỏ dựa trên vị trí ký tự tuyệt đối."""
    from docx.oxml.ns import qn
    from docx.text.run import Run
    for target_idx in split_indices:
        p_elem = p._element
        current_len = 0
        for child in list(p_elem.iterchildren()):
            ...
            r = Run(child, p)
            c_len = len(r.text)
            if current_len < target_idx < current_len + c_len:
                split_pos = target_idx - current_len
                t1 = r.text[:split_pos]
                t2 = r.text[split_pos:]
                r.text = t1
...
def _normalize_paragraph_prefix(p, current_q_num):
    """Chuẩn hoá prefix câu hỏi cho Tiếng Anh (vd: Câu 1: -> Question 1.)."""
    text = p.text
    ...
    if runs_to_modify:
        ...
        if len(first_text) >= total_replace_len:
            first_run.text = f"Question {current_q_num}. " + first_text[total_replace_len:]
        else:
            first_run.text = f"Question {current_q_num}. "
...
def _split_inline_answers(doc, p, text: str, is_lower: bool = False) -> bool:
    ...
    # Nếu chỉ có 1 nhãn trắc nghiệm:
    # - Nếu nhãn đó nằm ở đầu dòng thì không cần tách
    # - Nếu nhãn đó không phải là B/b (ví dụ C, D, F...) thì chắc chắn không phải là dòng trắc nghiệm bị thiếu A, mà là văn bản bình thường (ví dụ: true (T) or false (F).)
    if len(matches) == 1:
        if len(prefix_text) == 0:
            return False
        clean_lbl = re.sub(r'[* ]', '', first_match.group(1)).upper()
        if clean_lbl not in ['B']:
            return False
        
    # Kiểm tra xem phương án đầu tiên (A/a) có bị thiếu nhãn không
    # Nếu có văn bản trước nhãn B/b (hoặc lớn hơn) và văn bản đó không bắt đầu bằng A/a
    is_first_missing = False
    first_label = re.sub(r'[* ]', '', first_match.group(1)).upper()
    if len(prefix_text) > 0 and first_label in ['B', 'C', 'D', 'E', 'F', 'G', 'H']:
        if not re.match(r'^(?:\()?A\s*[.:)/\-]\s*', prefix_text, re.IGNORECASE):
            is_first_missing = True
            missing_prefix = "a. " if is_lower else "A. "
    ...
    if not split_indices and not is_first_missing:
        return False
    
    # Tiền xử lý để tách các run chứa nhiều nhãn
    _preprocess_split_runs(p, split_indices)
    ...
    def create_split_para():
        ...
        if pPr is not None:
            ...
            # Xóa bỏ thuộc tính tự động đánh số danh sách để tránh bị lặp nhãn (ví dụ: B. B. d-a-b-c)
            numPr = pPr_copy.find(qn('w:numPr'))
            if numPr is not None:
                pPr_copy.remove(numPr)
            new_p._element.append(pPr_copy)
        return new_p
        
    current_p = create_split_para()
    new_paragraphs.append(current_p)
    
    # Tính toán lại offset sau khi tách run
    current_len = 0
    child_starts = {}
    for child in p_elem.iterchildren():
        child_starts[child] = current_len
        if child.tag == qn('w:r'):
            r = Run(child, p)
    ...
    for child in list(p_elem.iterchildren()):
        ...
        if label_idx < len(split_indices) and child_starts[child] == split_indices[label_idx]:
            current_p = create_split_para()
            new_paragraphs.append(current_p)
            label_idx += 1
    ...
    if len(new_paragraphs) <= 1 and not is_first_missing:
        return False
        
    # Tự động chèn nhãn bị thiếu vào đầu đoạn văn thứ nhất
    if is_first_missing:
        first_p = new_paragraphs[0]
        added = False
        for r_elem in first_p._element.findall(qn('w:r')):
            t_el = r_elem.find(qn('w:t'))
            if t_el is not None:
                t_el.text = missing_prefix + (t_el.text or "")
                added = True
                break
        if not added:
            r_elem = OxmlElement('w:r')
            t_el = OxmlElement('w:t')
            t_el.text = missing_prefix
            r_elem.append(t_el)
            first_p._element.append(r_elem)
            
    for new_p in new_paragraphs:
        # Xóa các khoảng trắng/tab ở đầu văn bản của run đầu tiên để tránh bị "thò thụt" lệch lề
        for r_elem in new_p._element.findall(qn('w:r')):
            has_text = False
...
def auto_wrap_english_groups(doc):
    """Tự động bọc [CỐ ĐỊNH], [ĐẢO CÂU], [HẾT NHÓM] cho môn Tiếng Anh."""
    import re
    from app.core.word_parser import iter_block_items
    from docx.text.paragraph import Paragraph
    from docx.table import Table
    
    # Các từ khóa nhận diện lệnh Tiếng Anh
    fixed_keywords = [
        r"listen to", r"blanks", r"numbered blanks", r"announcement", r"advertisement", 
        r"write a paragraph", r"write an essay", r"write an email", r"write a letter", 
        r"write a passage", r"complete the sentences", r"rewrite the following"
    ]
    shuffle_keywords = [r"read the following passage", r"read the passage", r"answer the questions"]
    instruction_keywords = fixed_keywords + shuffle_keywords + [r"mark the letter"]
    
    BLANK_PATTERN = re.compile(r'\(\s*\d+\s*\)\s*(?:_{2,}|\.{2,}|…{2,}|…|…\.)')
    end_patterns = re.compile(r'^(=== THE END ===|THE END|HẾT|ANSWER KEY|BẢNG ĐÁP ÁN)', re.IGNORECASE)
    
    in_group = False
    has_seen_question = False
    current_group_tag = "[ĐẢO CÂU]"
    
    def is_section_header(text):
        if not text.strip(): return False
        if SECTION_RE.match(text.strip()):
            return True
        if end_patterns.match(text.strip()):
            return True
        return False
    ...
    for block in blocks:
        if isinstance(block, Paragraph):
            ...
            # Nếu đang ở trong một nhóm, kiểm tra xem có nên đóng nhóm không
            if in_group:
                # Nhóm kết thúc khi gặp một instruction mới HOẶC một section mới
                if is_section_header(text) or (is_instruction(text) and not QUESTION_RE.match(text)):
                    new_p = block.insert_paragraph_before("[HẾT NHÓM]")
                    for r in new_p.runs: r.bold = True
                    in_group = False
                    has_seen_question = False
                # Hoặc khi gặp một đoạn văn của bài đọc điền từ mới (cloze test) bắt đầu
                elif has_seen_question and not QUESTION_RE.match(text) and not PLAIN_NUMBER_RE.match(text) and BLANK_PATTERN.search(text):
                    new_p = block.insert_paragraph_before("[HẾT NHÓM]")
                    for r in new_p.runs: r.bold = True
                    in_group = False
                    has_seen_question = False
            ...
            # Nếu chưa ở trong nhóm, kiểm tra xem có phải là bài đọc / instruction mới bắt đầu nhóm mới không
            if not in_group:
                if is_instruction(text) and not QUESTION_RE.match(text):
                    current_group_tag = get_group_type(text)
                    new_p = block.insert_paragraph_before(current_group_tag)
                    for r in new_p.runs: r.bold = True
                    in_group = True
                    has_seen_question = False
                elif not QUESTION_RE.match(text) and not PLAIN_NUMBER_RE.match(text) and BLANK_PATTERN.search(text):
                    current_group_tag = "[CỐ ĐỊNH]"
                    new_p = block.insert_paragraph_before(current_group_tag)
                    for r in new_p.runs: r.bold = True
                    in_group = True
                    has_seen_question = False
            ...
            # Chỉ tự động ngắt nhóm khi gặp bảng 1x1 (thường là khung chứa bài đọc / announcement / advertisement)
            # Không ngắt nhóm cho các bảng khác (như bảng True/False nhiều hàng cột, hoặc bảng lựa chọn)
            is_1x1_table = (len(block.rows) == 1 and len(block.columns) == 1)
            if is_1x1_table and in_group and has_seen_question:
                new_p_end = doc.add_paragraph("[HẾT NHÓM]")
                for r in new_p_end.runs: r.bold = True
                block._element.addprevious(new_p_end._element)

                new_p_start = doc.add_paragraph(current_group_tag)
                for r in new_p_start.runs: r.bold = True
                block._element.addprevious(new_p_start._element)

                has_seen_question = False
                
    if in_group:
        # Nếu đến cuối file mà vẫn đang trong nhóm, đóng nhóm
        new_p = doc.add_paragraph("[HẾT NHÓM]")
        for r in new_p.runs: r.bold = True


def break_tf_tables_to_paragraphs(doc):
    """Tìm và phá các bảng True/False thành văn bản trắc nghiệm."""
    import re
    # Quét tất cả các bảng, NHƯNG phải bỏ qua bảng đáp án ở cuối đề!
    # Thường bảng đáp án nằm ở cuối cùng. Hoặc chúng ta chỉ phá các bảng có chứa 'Question'.
    
    tables_to_remove = []
    for table in doc.tables:
        # Bỏ qua nếu là bảng trắc nghiệm có chứa các nhãn trắc nghiệm inline (A. B. C. D.)
        has_choices = False
        for row in table.rows:
            for cell in row.cells:
                if INLINE_MC_RE.search(cell.text):
                    has_choices = True
                    break
            if has_choices: break
            
        if has_choices:
            continue

        is_tf_table = False
        # Kiểm tra xem bảng có chứa câu hỏi không
        for row in table.rows:
            for cell in row.cells:
                if re.match(r'^(Câu|Question|Bài|câu|bài)\s*(hỏi\s+)?(\d+)', cell.text.strip(), re.IGNORECASE):
                    is_tf_table = True
                    break
            if is_tf_table: break
        ...
        # Chèn văn bản trước bảng
        p_element = table._element
        parent = p_element.getparent()
        
        # Lấy index của bảng trong parent
        ...
        for row in table.rows:
            ...
            for cell in row.cells:
                text = cell.text.strip()
                if re.match(r'^(Câu|Question|Bài|câu|bài)\s*(hỏi\s+)?(\d+)', text, re.IGNORECASE):
                    q_text = text
                    break
            ...
            if q_text:
                ...
                # Tạo phương án A. True
                new_p_a = OxmlElement('w:p')
                parent.insert(idx, new_p_a)
                p_a = Paragraph(new_p_a, doc)
                p_a.text = "A. True"
                idx += 1
                
                # Tạo phương án B. False
                new_p_b = OxmlElement('w:p')
                parent.insert(idx, new_p_b)
                p_b = Paragraph(new_p_b, doc)
                p_b.text = "B. False"
                idx += 1
...
def normalize_english_docx(input_path: str, output_path: str, options: dict) -> Tuple[bool, int, str]:
    ...
    try:
        ...
        # 0.1 Chuẩn hoá các ngoại lệ đặc biệt (Tiêu đề Tiếng Anh, lỗi gõ F).)
        for p in get_all_paragraphs(doc):
            text = p.text.strip()
            if not text: continue
            
            # Nếu đoạn văn bắt đầu bằng F). (thường do lỗi gõ phím của giáo viên trước bài True/False)
            if re.match(r'^F\)\.\s', text):
                for run in p.runs:
                    if 'F).' in run.text:
                        run.text = run.text.replace('F).', '(F).', 1)
                        break
            
            # Nếu đoạn văn là tiêu đề tiếng anh (VD: C. READING, B. LEXICO - GRAMMAR)
            # Chỉ khớp nếu toàn bộ phần chữ phía sau là IN HOA (A-Z)
            elif re.match(r'^[A-D][./]\s+([A-Z\s\-]+\.?)$', text):
                # Thay thế run đầu tiên chứa tiền tố A., B., C., D.
                replaced = False
                for run in p.runs:
                    if re.match(r'^[A-D][./]\s*', run.text.lstrip()):
                        run.text = re.sub(r'^([A-D])[./]\s*', r'PHẦN \1. ', run.text.lstrip(), 1)
                        replaced = True
                        break
                # Nếu tiền tố nằm rải rác, fallback bằng cách sửa trực tiếp đoạn văn (mất định dạng một chút)
                if not replaced:
                    p.text = re.sub(r'^([A-D])[./]\s*', r'PHẦN \1. ', p.text)

        # 0. Chuyển đổi các bảng chứa các phương án trắc nghiệm thành đoạn văn
        convert_choice_tables_to_paragraphs(doc)

        # 0.1 Chuyển đổi các bảng chứa câu hỏi (nhưng không phải bảng True/False) thành đoạn văn
        convert_all_question_tables_to_paragraphs(doc)

        # 0.25 Phá bảng True/False
        break_tf_tables_to_paragraphs(doc)

        # 0.27 Tách các đoạn văn vừa lấy ra từ bảng nếu có chứa xuống dòng (\n hoặc w:br)
        split_paragraphs_at_br(doc)

        # 0.5 Tự động bọc nhóm Tiếng Anh
        auto_wrap_english_groups(doc)
        ...
        # Kiểm tra liên tục: nếu phần sau chứa (max phần trước + 1) thì chắc chắn là liên tục
        for i in range(1, 10):
            if sec_questions.get(i) and sec_questions.get(i+1):
                max_prev = max(sec_questions[i])
                if (max_prev + 1) in sec_questions[i+1]:
                    continue # Vẫn liên tục
                elif min(sec_questions[i+1]) <= max_prev:
                    is_already_continuous = False
                    break
            
        offsets = {i: 0 for i in range(1, 20)}
        if not is_already_continuous:
            current_offset = 0
            for i in range(2, 20):
                if sec_questions.get(i-1):
                    current_offset += max(sec_questions[i-1])
        ...
        while p_idx < len(paragraphs):
            ...
            if current_q_num > 0 and elem_id not in split_done_ids:
                if _split_inline_answers(doc, p, p.text, is_lower=False):
        ...
        if current_section_idx >= 3 or key_count > 0:
            ...
            while p_idx < len(paragraphs):
                ...
                if q_match:
                    ...
                    if correct_val and is_short_answer_value(correct_val):
